...
- [ ] Current `index.js` backup (`index.legacy.js`)
- [ ] সব API endpoint list + Postman/Thunder Client collection তৈরি
- [ ] Frontend `VITE_API_URL` endpoints document করা
- [ ] `.env.example` তৈরি (secrets ছাড়া)
- [ ] Git branch তৈরি: `refactor/mvc-mongoose`
...
- [ ] `src/` folder structure তৈরি
- [ ] `config/env.js` — required env vars validate
- [ ] `config/firebase.js`, `config/stripe.js` extract
- [ ] `app.js` — express, cors, helmet, json parser
- [ ] `middleware/auth.middleware.js` — `verifyJWT` move
- [ ] `middleware/role.middleware.js` — admin/seller move
- [ ] `utils/ApiError.js`, `catchAsync.js`, `sendResponse.js`
- [ ] `middleware/error.middleware.js` + `notFound.middleware.js`
- [ ] `src/index.js` (Vercel) + `src/server.js` (local)
- [ ] `vercel.json` → `src/index.js` point করা
